---
name: scaffold-memory-bank
description: Interactively scaffold a memory bank — a structured knowledge layer of decisions, rules, exceptions, and context — into the current repo or a separate bank repo. Walks scope, location, layout, agent integration, and optional hydration skill install.
agent: agent
---

<!-- Kept in sync with .claude/commands/scaffold-memory-bank.md and the body inside the four-backtick fence in SCAFFOLD.md (repo root). Change once, replicate to all three. -->
