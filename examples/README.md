...
This directory contains a worked example memory bank for a fictional developer platform. It illustrates the architecture defined in the [`model/`](../model/) directory: what the four record types look like in practice, and how the lifecycle states and typed relationships connect them.
...
This chain illustrates how a Context provides the environmental fact that justifies an Exception, which in turn authorizes a Decision:
...
If CTX-0005 ever changed (say the SLA was relaxed to 25ms), the agent should surface EXC-0002 as a record that depends on it and may need to be revisited.
...
There's no `_templates/` directory here either; these examples assume you already know the schema. Use [SCAFFOLD.md](../SCAFFOLD.md) to generate templates for your own project.
