# PM / Product Owner Authorship Guide

This guide walks through when you author memory bank records as a product manager or product owner, and what each type looks like in your voice.
...
**Writing a commitment as a Decision.** A Decision is a choice you could have made differently. A commitment is a promise already made. Commitments are Contexts (with a `commitment` tag), not Decisions. You might author a Decision *downstream* of a commitment ("given CTX-0032, we decided X"), but the commitment itself is the Context.
